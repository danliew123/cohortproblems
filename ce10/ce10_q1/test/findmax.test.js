...
describe("testing findmax", () => {
  ...
  test("a test that passes", () => {
    ...
    const length = 1000;
    const input = [max];
    for (let i = 0; i < length - 1; i++) input.push(Math.random() * max);
    expect(findmax(input)).toBe(max);
  });
  // empty array (should fail as return null when cannot find max)
  test("empty array", () => {
    result = findmax([]);
    expect(result).toBe(null);
  });
  // array with null values (should pass)
  test("array with null values", () => {
    result = findmax([null, null, null]);
    expect(result).toBe(null);
  });
  // array with NaN values (should fail as findmax doesn't return null unlike in expected specs)
  ...
  test("arrays with more than one ordinal value and the max value is at index 0", () => {
    result = findmax([67, 32, 14]);
    expect(result).toBe(67);
  });
  // arrays with more than one ordinal value and the max value is at index N-1, where N is the length.
  test("arrays with more than one ordinal value and the max value is at index N-1, where N is the length", () => {
